...
c includes image flags in data file so chains can be unraveled later
 ...
 905  format(a,i1,a,i5)
 906  format(a,f5.3)
 907  format(a,i1,a,f5.3)
...
c read chain definitions
      ...
      allocate(natomsset(nsets))
      allocate(nchainset(nsets))
      allocate(actualmolfrac(nsets))
...
c calculate volume of box
      ...
      yboundhi = -yboundlo
      zboundlo = 0.0
      zboundhi = zboundlo+zprd

c calculate number of monomers at desired total density

      atoms = rhostar*volume

c calculate the total number of atoms, looping over sets of species
      natoms = 0
      nchains = 0
      iset = 1
      denomsum = 0
      ...
      do j = iset+1,nsets
         denomsum = denomsum + molfrac(j)*nmonomer(j)
      enddo 
     
      denom = nmonomer(iset) + denomsum/molfrac(iset)
      nchainset(iset) = INT(atoms/denom)
      natomsset(iset) = nchainset(iset)*nmonomer(iset)
      natoms = natoms + natomsset(iset)
      nchains = nchains + nchainset(iset) 
     
      write(6,905) '# Number of chains of Species ',iset,' = ',
     $nchainset(iset)

      if (nsets > 1) then 
         do jset = iset+1,nsets
            nchainset(jset) = INT((molfrac(jset)/molfrac(iset))
     $      *nchainset(iset))
            natomsset(jset) = nchainset(jset)*nmonomer(jset)
            natoms = natoms + natomsset(jset)
            nchains = nchains + nchainset(jset) 
            write(6,905) '# Number of chains of Species ',jset,' = ',
     $      nchainset(jset) 
         enddo
      endif
      ...
      do kset = 1,nsets 
         actualmolfrac(kset) = REAL(nchainset(kset))/REAL(nchains) 
         write(6,907) '# Actual mol fraction of Species ',kset,' = ',
     $   actualmolfrac(kset)
      enddo
      
      write(6,900)

      rhostar = REAL(natoms/volume) 
      write(6,906) '# Actual reduced density, rhostar = ',rhostar
...
c random starting point for the chain in the box
 ...
 20       x1 = 0.0
          ...
          if (z2 < (zboundlo+1)) then
            goto 20
          elseif (z2 > (zboundhi-1)) then
            goto 20
          endif
...
c if the new point is outside the simulation box, then choose another point
            ...
            if (x2 < xboundlo) then
              goto 10
            else if (x2 >= xboundhi) then
              goto 10
            else if (y2 < yboundlo) then
              goto 10
            else if (y2 >= yboundhi) then 
              goto 10
            else if (z2 < zboundlo+0.5) then 
              goto 10
            else if (z2 >= zboundhi-0.5) then 
              goto 10
            endif 
...
c compute quantities needed for LAMMPS file
      ...
      nbondtypes = 0
      do iset = 1,nsets
        nbonds = nbonds + nchainset(iset)*(nmonomer(iset)-1)
...
c write out LAMMPS file
      ...
      write (6,900) '# LAMMPS polymer melt data file'
      write (6,*)
...
c in the conditional with the MOD operator,
c specify the first atom of the bonding pair 
c that you wish to cleave 
c                if (MOD(imonomer,nmonomer(iset))==20) then
c                  write (6,904) m,2,n,n+1
c                else
c                  write (6,904) m,nbondtype(iset),n,n+1
c                endif
              endif
            enddo
          enddo
