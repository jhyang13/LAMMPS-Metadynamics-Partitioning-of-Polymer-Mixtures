...
int main(int argc, char *argv[])
{
  ...
  out13=fopen(partition, "w");
	...
	fprintf(out1, "# timestep  xRg^2        yRg^2           zRg^2           Rg,xy^2          xRe^2         yRe^2         zRe^2          Re^2          xL^2           yL^2          zL^2           L^2\n");
...
int readconfig(double **positions, int nchains, int chainlength)
{
	...
	/*fgets(line, 1024, fp);
	puts(line);*/
	for(surface=0; surface<natoms-chainlength*nchains; surface++)
	{
	    fscanf(fp, "%d %d %lg %lg %lg\n", &surfatomno, &surfatomtype, &surfx, &surfy, &surfz); 	    
	    
	   /*printf("Atom type of atom %d: %d\n", surfatomno, surfatomtype);*/
	   /* if (atomtype==1)
	      {
...
int readheader(int *timestep, int *natoms, double box[3][2], int nchains, int chainlength)
{
	...
	fgets(line, 1024, fp);
	/*printf("%s\n", line);*/
	if ( strcmp(line, "ITEM: ATOMS id mol type x y z ix iy iz\n") != 0)
	{
		return(-4);
	}
...
int readatom(double *atom, double xbox, double ybox, double zbox, int *atomtype)
{
	...
	fscanf(fp, "%d %d %d %lg %lg %lg %d %d %d\n",
	       &atomno, &molid, atomtype, &x, &y, &z, &image[0], &image[1], &image[2]);
